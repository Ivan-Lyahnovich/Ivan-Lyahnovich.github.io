...
/* Добавлены getCenterX, getCenterY, setCenterX, and setCenterY */
...
Game.MovingObject = function(x, y, width, height, velocity_max = 15) {
  ...
  this.jumping      = false;
  this.velocity_max = velocity_max;// добавлен speed_max, скорость не превышает 16
  this.velocity_x   = 0;
  this.velocity_y   = 0;
  this.x_old        = x;
  this.y_old        = y;
...
/* Я добавил setCenterX, setCenterY, getCenterX, and getCenterY */
...
Game.Door.prototype = {
 ...
 /* Проверка на столкновение между переходным объектом и персонажем. */
...
Game.Player.prototype = {
  ...
  jump: function() {
    ...
    /* Ограничение в прыжке при падении не быстрее 10px за кадр */
  ...
  updatePosition:function(gravity, friction) {
    ...
    /* Ограничение скорости velocity_max */
...
Game.TileSet = function(columns, tile_size) {
  ...
  this.frames = [new f( 112, 80, 16, 16, 1, -4), // смотрит на лево
                 new f( 32,  80, 16, 16, 1, -4), // прыжок в левую сторону
                 new f( 96,  80, 16, 16, 1, -4), new f(80, 80, 16, 16, 1, -4), new f(64, 80, 16, 16, 1, -4), new f(32, 80, 16, 16, 1, -4), // ходьба в левую сторону
                 new f(  0,  96, 16, 16, 1, -4), // смотрит на право
                 new f( 80,  96, 16, 16, 1, -4), // прыжок в правую сторону
                 new f( 16,  96, 16, 16, 1, -4), new f(32, 96, 16, 16, 1, -4), new f(48, 96, 16, 16, 1, -4), new f(16, 96, 16, 16, 1, -4) // ходьба в правую сторону
                ];       
};
...
Game.World = function(friction = 0.85, gravity = 2) {
  ...
  this.player    = new Game.Player(80, 16);

  this.zone_id   = "00";// Текущая зона.

  this.doors     = [];// Массив переходов на уровне.
  this.door      = undefined; // При переходе игрока, игра установит это свойство и уровень будет загружен.
...
Game.World.prototype = {
  ...
  collideObject:function(object) {
    ...
    /* Убрано столкновение с мировой границей. */
  ...
  /* Функция установки берет объект зоны, сгенерированный из файла zoneXX.json. 
     Это устанавливает все мировые значения на значения зоны. Если игрок только что прошел через
     зону перехода, он использует переменную this.door. */
  setup:function(zone) {

    /* Получение новой карты тайлов, зоны и сброс массива перехода. */
    ...
    /* Если игрок выполнил переход, this.door будет ссылаться на него. */
    if (this.door) {

      /* Пункт назначения равен -1, это значит, что он не будет использоваться. Так как каждая зона
       охватывает от 0 до его ширины и высоты, любое отрицательное число будет недействительным. Если
       назначение двери не равно -1, игрок сохранит свою текущую позицию для этой оси.*/
      if (this.door.destination_x != -1) {

        this.player.setCenterX   (this.door.destination_x);
        this.player.setOldCenterX(this.door.destination_x);// сброс старой позиции.
      ...
      this.door = undefined;// Сброс this.door, для не вызывания загрузку зоны.
  ...
  update:function() {
    ...
    /* Проходим через все переходы в текущей зоне и проверяем действие. 
       Если столкновение с одним, то устанавливаем переменную мировых переходов равной этой двери, 
       поэтому мы знаем, как использовать ее для загрузки следующей зоны. */
